...
#     history = model.fit(st_train_data,train_labels, epochs=epochs, validation_split=0.2, verbose=1, callbacks=[early_stop])
    history = model.fit(st_train_data,train_labels, epochs=epochs, validation_split=0.2, verbose=0 ,callbacks=[early_stop]) 
    ...
    early_stop = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=5)
    history = model.fit(x_train, y_train, epochs=500, batch_size=64, verbose=0 ,callbacks=[early_stop])
...
#      
    ...
    enterclass = request.GET.get('enterclass')
    checkcount = request.GET.get('checkcount')
    discount = request.GET.get('discount')
    
    
    print(raisehandcount,discount,enterclass,checkcount)
    new_data = np.array([[raisehandcount,enterclass,checkcount,discount]]).astype(float)
    ...
    yh_fig3.update_layout(width=700,paper_bgcolor='rgba(255,255,255,0.2)',
                          plot_bgcolor='rgba(255,255,255,0.2)',
                          font=dict(color='#000',size=30))
