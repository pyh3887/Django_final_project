...
//		            mae = "<h4>" + data['mae'] + "</h4><br><br>"
		            ...
		            linear = "<p style='font-size: 50px;'>Linear Regression</p>"
		            logit = "<p style='font-size: 43px;'>Logistic Regression</p>"
...
//		               모델을 최적화하기 위해서는 모델의 성능을 평가할 수 있는 척도가 필요하며, 모델이 예측한 값과 관측값 사이의 불일치 정도를 확인하려면 '거리'를 반영하는 척도가 필요하다. 이러한 거리를 loss(손실) 함수라고 하며, 이 함수의 값을 최소화하는 파라미터(가중치, 편향)을 찾아내는 것이 딥러닝 모델을 최적화하는 것이다.<br>\
		            ...
		            msetext = "<h4>MSE- mean squared error - 평균 제곱 오차 <br>\
		                         ...
		                         1. 수학적인 분석이 쉽다 <br>2. 계산의 용이하다 등<br>\
		                         MSE는 손실함수로써 사용된다.</h4>"
		            ...
		            logitgo = "<a href='#' id='logitgo' style='color: white; text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black; text-decoration: none;'>Learn more</a>"
...
//		            $('#logittext').append(logittext2)
		   ...
		   $('body').on('click', '.logitgo', function(event) {
		      $('#logitgo').detach()
		      $('#logittext').append(logittext1)
		       $('#logittext').append(acc)
		       $('#logittext').append(yh)
